...
특정 클래스 안에서 주로 사용되는 클래스를, 해당 클래스 내부에 선언한 것을 의미합니다. 장점으로는 외부 클래스의 멤버들에 쉽게 접근할 수 있습니다.
* `종류`
  * instance 클래스 : 인스턴스 필드 위치에 선언하며, 주로 외부 클래스의 인스턴스 멤버들과 관련된 작업에 사용될 목적으로 선언됩니다. 
  * static 클래스 : 외부 클래스의 static 멤버처럼 다뤄지며, static 메서드에서 사용될 목적으로 선언됩니다.
  * anonymous(익명) 클래스 : 선언과 동시에 객체를 생성하는 일회용 클래스 입니다
  * local 클래스 : 외부 클래스의 메서드나 초기화 블럭 안에 선언하며 선언된 영역의 내부에서만 사용됩니다.

* `제어자와 접근성` (refer to InnerEx2.class)
  * 일반 변수에 사용할 수 있는 접근제어자와 동일합니다.
  * static 클래스에서만 static 멤버를 정의할 수 있습니다. 단, final static 의 상수 취급함으로 모든 클래스에서 선언 가능합니다.
  * instance 메서드에서는 instance / static 멤버 모두 접근이 가능합니다.
  * 메서드 내에서 지역적으로 선언된 클래스는 해당 내부 영역에서만 참조할 수 있고, 외부 참조는 막혀 있습니다.

### 4. 익명 클래스(anonymous class)
이름이 없는 일회용 클래스입니다. 특징으로는 아래와 같습니다.
* 선언과 생성을 동시에 합니다.
* 하나의 객체만 생성 가능합니다.
