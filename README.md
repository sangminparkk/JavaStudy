...
### 6. 멤버간의 참조와 호출
같은 클래스 내에서는 객체 생성이나 참조변수없이 참조할 수 있습니다.   
다만, static 멤버들(변수나 메서드)은 인스턴스 멤버를 참조할 수 없습니다.
```java
public class MyMath {
    void instanceAdd() {}
    ...
    static void static2Add() {
        staticAdd();
        instanceAdd(); // 참조 불가 (컴파일에러)
    }
}
```
컴파일 에러가 발생하는 이유는 두가지 라고 생각합니다.
* `객체 생성의 필요성` : 인스턴스 메서드는 객체가 먼저 생성된 후에 사용 가능합니다. static 메서드 영역에서는 참조할 객체가 없기 때문에 컴파일 에러가 발생합니다.
* `메모리 생성 시점` : static 멤버들은 클래스가 로딩될때 함께 메모리에 로드됩니다. static 메서드가 호출되었다면 프로그램 시작 시점일텐데, 이때 인스턴스는 아직 생성되지 않았을 가능성이 있기 때문에 컴파일러는 잠재적 실행 오류를 방지하기 위해 인스턴스 메서드 호출을 금지합니다.
...
* 본질
  * 다양한 입력 데이터를 받아도 동일한 프로세스를 처리하고자 할때, 개발자 입장에서 같은 기능을 하는 메서드를 구분할 수 있도록 코드의 가독성을 높이는 방향으로 설계할 수 있다. 
...
## 객체지향개념 I-3
### 1. 생성자
생성자를 `인스턴스 초기화 메서드`라고 부르는 이유는, **객체를 생성하고 초기화**하는데 그 역할이 있기 때문입니다.   
* 모든 클래스에 반드시 하나 이상의 생성자가 있어야 합니다.
* 생성자의 이름은 클래스명과 동일해야 합니다.
* 생성자는 리턴타입을 갖지 않습니다. (void 역시 사용하지 않음)

### 1-1. 기본 생성자
매개변수가 없는 default 생성자이고, 클래스에서 생성자가 하나도 없으면 자바 컴파일러가 자동으로 기본생성자를 추가 해줍니다.

### 1-2. 파라미터 생성자
객체 생성과 동시에 데이터를 할당해줌으로 코드가 간결합니다. 또한 필드값이 초기화되지 않은 상태를 피할 수 있습니다.

### 1-3. this()
같은 클래스 내부에서 다른 생성자를 호출할때 사용합니다.  
아래 코드 기준, 기본생성자 블럭 내에서 파라미터 생성자를 호출하는 상황입니다. this(매개변수1,매개변수2)를 호출해서 데이터를 할당하는 역할을 합니다.  
이때, this()는 블럭 내 첫줄에 위치해야 하는데요. 이유는 자바 컴파일러가 **생성자를 호출하는 시점과 객체 초기화 순서를 보장**하기 위함입니다.
```java
public class Card {
...
```
※ super() : 상속관계를 다룰때 설명할 개념으로 부모클래스의 생성자를 호출할때 사용할때 사용합니다.

### 2. this ★
인스턴스 자기 자신을 가리키는 참조변수로, 인스턴스의 주소값이 저장되어 있습니다. 
* 모든 인스턴스 메서드에서 지역변수로 숨겨진 채로 존재하며
* 인스턴스 변수와 지역변수를 구별하기 위해 참조변수 this 를 사용합니다. 
```java
public Card(String kind, int number) {
    this.kind = kind; // this.kind : 인스턴스 변수, kind : 매개변수(지역변수)
    this.number = number;
}
```

### 3. 변수의 초기화
변수를 선언하고 처음으로 값을 저장하는 작업입니다.
* 멤버변수(인스턴스변수, 클래스변수) : 자동으로 초기화됨으로 초기화 생략 가능
  * Reference Type : null 초기화
    * ex) String, Integer : null
